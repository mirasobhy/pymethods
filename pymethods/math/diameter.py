...
def all_diameters(contour: np.array) -> np.float:
    ...
    for pnum, point in enumerate(centered):
        point = point[None, :]
        r_current = _np.linalg.norm(point)
        perp = _np.cross(point[None, :], normal)[0]
        inner = _np.inner(centered, perp)
        potential = []
        NTRIES = 10
        while len(potential) == 0:
            sortedProj = _np.argsort(_np.abs(inner.squeeze()))[0:NTRIES]
            sortedProj = [i for i in sortedProj if i != pnum]
            testingPoints = centered[sortedProj]
            test_points_inner = _np.inner(testingPoints, point)
            potential = test_points_inner[test_points_inner < 0]
            NTRIES += 1
        diameter = r_current + _np.abs(potential[0])
        all_diameters.append(diameter)
    return _np.array(all_diameters)
